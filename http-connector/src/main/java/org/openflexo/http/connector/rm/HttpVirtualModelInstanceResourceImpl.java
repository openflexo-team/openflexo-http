...
public abstract class HttpVirtualModelInstanceResourceImpl
	...
	@Override
	public List<HttpVirtualModelInstanceResource> getVirtualModelInstanceResources() {
		return getContents(HttpVirtualModelInstanceResource.class);
	}
	 ...
	 */
	@Override
	public List<HttpVirtualModelInstanceResource> getVirtualModelInstanceResources(VirtualModel virtualModel) {
		List<HttpVirtualModelInstanceResource> returned = new ArrayList<>();
		for (HttpVirtualModelInstanceResource vmiRes : getVirtualModelInstanceResources()) {
	...
	public String computeDefaultURI() {
		if (getContainer() != null) {
			return getContainer().getURI() + "/" + (getName().endsWith(HttpVirtualModelInstanceResourceFactory.HTTP_SUFFIX) ? getName()
					: getName() + HttpVirtualModelInstanceResourceFactory.HTTP_SUFFIX);
		}
		if (getResourceCenter() != null) {
			return getResourceCenter().getDefaultBaseURI() + "/" + (getName().endsWith(HttpVirtualModelInstanceResourceFactory.HTTP_SUFFIX)
					? getName() : getName() + HttpVirtualModelInstanceResourceFactory.HTTP_SUFFIX);
		}
		return null;
	}
