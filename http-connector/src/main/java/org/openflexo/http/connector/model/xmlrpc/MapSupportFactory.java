 ...
 */
public class MapSupportFactory implements ContentSupportFactory<MapSupport> {
	...
	public MapSupportFactory(String urlPropertyName) {
		this.urlPropertyName = urlPropertyName;
	}

	@Override
	public List<MapSupport> newSupports(HttpVirtualModelInstance owner, String path, InputStream stream, String pointer) {

		List<MapSupport> result = new ArrayList<>();
			...
			if (node instanceof ArrayNode) {
				for (JsonNode child : node) {
					if (child instanceof ObjectNode) {
						...
						result.add(new MapSupport(owner, childPath, childNode, null));
					}
				}
	...
	@Override
	public MapSupport newSupport(HttpVirtualModelInstance owner, String identifier, InputStream stream, String pointer) {
		JsonNode node = null;
		...
		return new MapSupport(owner, identifier, (ObjectNode) node, pointer != null ? JsonPointer.compile(pointer) : null);
	}
